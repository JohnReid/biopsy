...
#ifndef BIOPSY_NUMPY_CONVERTER_H_
#define BIOPSY_NUMPY_CONVERTER_H_
...
struct _converter
{
	...
	{
		throw std::logic_error( "convert_from_numpy not specialised for this type" );
	}

	static boost::python::object
	to_numpy( boost::python::object numpy, const CContainer & c_array, boost::python::object array_function )
	{
		throw std::logic_error( "convert_to_numpy not specialised for this type" );
	}
};
...
template< typename T >
struct _converter< std::vector< T > >
{
	...
	inline static void
	from_numpy( object numpy_array, vector & c_array )
	{
		using namespace boost::python;

		tuple shape( numpy_array.attr( "shape" ) );
		if( len( shape ) != 1 ) throw std::logic_error( "numpy::array must have 1 dimension for conversion to std::vector" );
	...
	inline static object
	to_numpy( object numpy, const vector & c_array, object array_function )
	{
		using namespace boost::python;
...
struct _converter< boost::multi_array< T, N > >
{
	...
	inline static bool increment_index( index & i, const multi_array & c_array )
	{
		...
		{
			++i[dim];
			if( unsigned(i[dim]) != c_array.shape()[dim] )
			{
				return true;
			}
			else
...
#endif //BIOPSY_NUMPY_CONVERTER_H_
